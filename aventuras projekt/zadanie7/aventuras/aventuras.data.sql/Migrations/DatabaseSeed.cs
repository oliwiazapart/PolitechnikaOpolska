...
using aventuras.common.Extensions;
...
namespace aventuras.data.sql.Migrations
{
    public class DatabaseSeed
    {
        ...
        private IEnumerable<Post> BuildPostList(IEnumerable<DAO.User> userList)
        {
        ...
        private IEnumerable<DAO.Rating> BuildRatingList(IEnumerable<Post> postList,
            IEnumerable<DAO.User> userList, IEnumerable<Comment> commentList)
        {
            var ratingList = new List<DAO.Rating>();
        ...
        private IEnumerable<DAO.Share> BuildShareList(IEnumerable<Post> postList,
            IEnumerable<DAO.User> userList)
        {
            var shareList = new List<DAO.Share>();
            postList.ToList().Shuffle();
            for (int i = 0; i < userList.Count(); i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    shareList.Add(new DAO.Share
                    {
